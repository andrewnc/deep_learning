...
def readLangs(lang1, lang2, reverse=False):
    ...
    lines1 = open('data/%s.txt' % lang1, encoding='utf-8').\
        read().strip().split('\n')
    lines2 = open('data/%s.txt' % lang2, encoding='utf-8').\
        read().strip().split('\n')
    lines = []
    for i in range(len(lines1)):
        lines.append(lines1[i]+"x&x"+lines2[i])

    # Split every line into pairs and normalize
    pairs = [[normalizeString(s) for s in l.split('x&x')] for l in lines]
...
input_lang, output_lang, pairs = prepareData('en', 'es', True)
...
class Encoder(nn.Module):
    ...
    def forward(self, input_variable):
        embedded = self.embed(input_variable).view(1,1,-1) # This is breaking
...
class Decoder(nn.Module):
    def __init__(self, input_size, hidden_size, target_vocab_size, n_layers=1, max_target_length=30):
        ...
        self.GRUCell = nn.GRU(self.hidden_size, self.hidden_size)
...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    for ei in range(input_length):
        print(input_variable.size(), input_variable[ei].size())
...
def trainIters(encoder, decoder, n_iters, print_every=100, plot_every=100, learning_rate=0.01):
    ...
    for i in range(1, n_iters + 1):
        ...
        if (i % print_every) == 0:
            ...
            print_loss_total = 0
            temp_out =  '%s (%d %d%%) %.4f' % (timeSince(start, i / float(n_iters)),
...
attn_decoder1 = Decoder(hidden_size, hidden_size, output_lang.n_words)

if use_cuda:
    encoder1 = encoder1.cuda()
    attn_decoder1 = attn_decoder1.cuda()

a = trainIters(encoder1, attn_decoder1, 500, print_every=100)
...
with open("output.txt", "w") as f:
    s = evaluateRandomly(encoder1, attn_decoder1)
    for line in s.split("\n"):
        f.write(line)
